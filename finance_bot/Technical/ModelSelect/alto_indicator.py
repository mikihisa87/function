...
def iATR(df, ma_period, ma_method='SMA'):
    TR = np.max(np.vstack((df['High'].values, shift(df['Close'].values))).T, axis=1)       - np.min(np.vstack((df['Low'].values, shift(df['Close'].values))).T, axis=1)
...
#Resist 関数
def iResist(df, period):
    resist = df["High"].rolling(window=period).max()
    df["Resist"] = np.zeros(len(df))
    df["Resist"][1:] = resist[:-1] 
    Resist = df["Resist"]
    return Resist

#Resist MA関数
def iResistMA(df, period):
    rma = df.rolling(window=period).mean()
    return rma

#Support 関数
def iSupport(df, period):
    resist = df["Low"].rolling(window=period).max()
    df["Support"] = np.zeros(len(df))
    df["Support"][1:] = resist[:-1] 
    Resist = df["Support"]
    return Resist

#Support MA関数
def iSupportMA(df, period):
    suma = df.rolling(window=period).mean()
    return suma

#Volume MA関数
def iVolumeMA(df, period):
    vma = df.rolling(window=period).mean()
    return vma
...
def money_flow(close_data, high_data, low_data, volume):
    mf = volume * typical_price(close_data, high_data, low_data)
    return mf

def iMFI(close_data, high_data, low_data, volume, period, Time):
    ...
    mfi_df = pd.concat([Time, mfi], axis=1)
    mfi_df = mfi_df.set_index("Time")
    mfi_df.rename(columns={0:"MFI"}, inplace=True)
    mfi= mfi_df["MFI"]
    return mfi